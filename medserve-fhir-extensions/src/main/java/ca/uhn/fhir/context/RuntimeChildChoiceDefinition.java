...
public class RuntimeChildChoiceDefinition extends BaseRuntimeDeclaredChildDefinition {
    ...
    static {
        System.out.println("WARNING :- HAPI class patch for RuntimeChildChoiceDefinition in play");
    }
     ...
     */
    RuntimeChildChoiceDefinition(Field theField, String theElementName, Child theChildAnnotation,
    ...
    void sealAndInitialize(FhirContext theContext,
        ...
        for (Class<? extends IBase> next : myChoiceTypes) {
            ...
            if (IBaseResource.class.isAssignableFrom(next) || IBaseReference.class.isAssignableFrom(next)) {
                //
                // System.out.println(
                // "Adding " + next.getName() + " to " + System.identityHashCode(myDatatypeToElementDefinition)
                // + " def " + nextDef.getName() + " " + System.identityHashCode(nextDef) + " as "
                // + theContext.getVersion().getResourceReferenceType().getName());
            ...
            if (myDatatypeToElementDefinition.containsKey(next)
                    && !nextDef.equals(myDatatypeToElementDefinition.get(next))) {
                throw new RuntimeException("Map already contains a key for " + next.getName()
                        + " trying to put definition " + nextDef + ". Existing definition for key is "
                        + myDatatypeToElementDefinition.get(next));
            }
            ...
            if (myDatatypeToElementName.containsKey(next)) {
                ...
                if (!existing.equals(elementName)) {
                    throw new ConfigurationException("Already have element name " + existing + " for datatype "
