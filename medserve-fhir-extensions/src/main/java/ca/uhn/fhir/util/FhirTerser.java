...
public class FhirTerser {
     ...
     * @param theIgnoreMissingFields The ignore fields in the target which do not exist (if false, an exception will be
     *            thrown if the target is unable to accept a value from the source)
     */
    public void cloneInto(IBase theSource, IBase theTarget, boolean theIgnoreMissingFields) {
        ...
        if (theSource instanceof IPrimitiveType<?>) {
            ...
            throw new DataFormatException("Can not copy value from primitive of type " + theSource.getClass().getName()
                    + " into type " + theTarget.getClass().getName());
        }
            ...
            for (IBase nextValue : nextChild.getAccessor().getValues(theSource)) {
                ...
                if (targetChild == null) {
                    ...
                    throw new DataFormatException(
                        "Type " + theTarget.getClass().getName() + " does not have a child with name " + elementName);
                }
     ...
     * message. Specifying a type of {@link IResource} would return the resource itself, as
     ...
     * but will not descend into linked resources (e.g.
     ...
     */
    public <T extends IBase> List<T> getAllPopulatedChildElementsOfType(IBaseResource theResource,
        ...
        visit(new IdentityHashMap<Object, Object>(), theResource, theResource, null, null, def, new IModelVisitor() {
            ...
            @Override
            public void acceptElement(IBaseResource theOuterResource, IBase theElement, List<String> thePathToElement,
                    BaseRuntimeChildDefinition theChildDefinition, BaseRuntimeElementDefinition<?> theDefinition) {
    ...
    private BaseRuntimeChildDefinition getDefinition(BaseRuntimeElementCompositeDefinition<?> theCurrentDef,
    ...
    @SuppressWarnings("unchecked")
    private <T> List<T> getValues(BaseRuntimeElementCompositeDefinition<?> theCurrentDef, Object theCurrentObj,
            List<String> theSubList, Class<T> theWantedClass) {
        ...
        if (name.startsWith("extension('")) {
            ...
            if (theSubList.size() > 1) {
                ...
                for (T nextElement : values) {
                    BaseRuntimeElementCompositeDefinition<?> nextChildDef =
                            (BaseRuntimeElementCompositeDefinition<?>) myContext
                                .getElementDefinition((Class<? extends IBase>) nextElement.getClass());
                    List<T> foundValues = getValues(nextChildDef, nextElement, theSubList.subList(1, theSubList.size()),
                        theWantedClass);
        ...
        if (theSubList.size() == 1) {
            ...
            for (IBase nextElement : values) {
                BaseRuntimeElementCompositeDefinition<?> nextChildDef =
                        (BaseRuntimeElementCompositeDefinition<?>) myContext
                            .getElementDefinition(nextElement.getClass());
                List<T> foundValues =
                        getValues(nextChildDef, nextElement, theSubList.subList(1, theSubList.size()), theWantedClass);
                retVal.addAll(foundValues);
            }
        }
        return retVal;
     ...
     * @param theTarget The owner of the compartment. Note that both the resource type and ID must be filled in on this
     *            IIdType or the method will throw an {@link IllegalArgumentException}
     ...
     */
    public boolean isSourceInCompartmentForTarget(String theCompartmentName, IBaseResource theSource,
            IIdType theTarget) {
        Validate.notBlank(theCompartmentName, "theCompartmentName must not be null or blank");
        Validate.notNull(theSource, "theSource must not be null");
        Validate.notNull(theTarget, "theTarget must not be null");
        Validate.notBlank(defaultString(theTarget.getResourceType()),
            "theTarget must have a populated resource type (theTarget.getResourceType() does not return a value)");
        Validate.notBlank(defaultString(theTarget.getIdPart()),
    ...
    private void visit(IBase theElement, BaseRuntimeChildDefinition theChildDefinition,
            BaseRuntimeElementDefinition<?> theDefinition, IModelVisitor2 theCallback,
        ...
        theCallback.acceptElement(theElement, Collections.unmodifiableList(theContainingElementPath),
        ...
        if (theElement instanceof ISupportsUndeclaredExtensions) {
            ...
            for (ExtensionDt nextExt : containingElement.getUndeclaredExtensions()) {
                ...
                theCallback.acceptUndeclaredExtension(nextExt, theContainingElementPath, theChildDefinitionPath,
        ...
        switch (theDefinition.getChildType()) {
                        ...
                        for (IBase nextValue : values) {
                            ...
                            if (childElementDef == null) {
                                ...
                                for (Iterator<String> iter =
                                        new TreeSet<String>(nextChild.getValidChildNames()).iterator(); iter
                                            .hasNext();) {
                            ...
                            if (nextChild instanceof RuntimeChildDirectResource) {
                                ...
                                theCallback.acceptElement(nextValue,
                                ...
                                visit(nextValue, nextChild, childElementDef, theCallback, theContainingElementPath,
                                    theChildDefinitionPath, theElementDefinitionPath);
                            }
                        }
                    }
                }
            ...
            case CONTAINED_RESOURCES: {
                ...
                for (IResource next : value.getContainedResources()) {
                    ...
                    visit(next, null, def, theCallback, theContainingElementPath, theChildDefinitionPath,
                        theElementDefinitionPath);
                }
                break;
            }
            ...
            case CONTAINED_RESOURCE_LIST: {
                if (theElement != null) {
                    ...
                    visit(theElement, null, def, theCallback, theContainingElementPath, theChildDefinitionPath,
                        theElementDefinitionPath);
                }
                break;
            }
    ...
    void visit(IBaseResource theResource, IModelVisitor2 theVisitor) {
        BaseRuntimeElementCompositeDefinition<?> def = myContext.getResourceDefinition(theResource);
        visit(theResource, null, def, theVisitor, new ArrayList<IBase>(), new ArrayList<BaseRuntimeChildDefinition>(),
            new ArrayList<BaseRuntimeElementDefinition<?>>());
    }
    ...
    private void visit(IdentityHashMap<Object, Object> theStack, IBaseResource theResource, IBase theElement,
        ...
        switch (def.getChildType()) {
            ...
            case COMPOSITE_DATATYPE: {
                ...
                for (BaseRuntimeChildDefinition nextChild : childDef.getChildrenAndExtension()) {
                    System.out.println("nextChild " + nextChild.getElementName() + " theElement " + theElement
                            + " path " + pathToElement);
